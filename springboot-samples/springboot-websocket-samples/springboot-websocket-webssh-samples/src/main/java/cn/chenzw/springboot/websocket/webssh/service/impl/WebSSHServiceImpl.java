package cn.chenzw.springboot.websocket.webssh.service.impl;

import cn.chenzw.springboot.websocket.webssh.constant.ConstantPool;
import cn.chenzw.springboot.websocket.webssh.domain.SSHConnectInfo;
import cn.chenzw.springboot.websocket.webssh.domain.WebSSHData;
...
public class WebSSHServiceImpl implements WebSSHService {
    ...
    public void initConnection(WebSocketSession session) {
        ...
        WebSSHRecorder recorder = new WebSSHRecorder(new WebSSHRecordLocalDiskStorage(new File("test2.cast")));
        recorder.start();
        sshConnectInfo.setRecorder(recorder);

        //将这个ssh连接信息放入map中
        sshMap.put(uuid, sshConnectInfo);

        WebSSHData webSSHData = new WebSSHData();
        webSSHData.setHost("192.168.17.231");
        webSSHData.setPort(22);
        webSSHData.setUsername("weblogic");
        webSSHData.setPassword("itm.123");
    ...
    public void close(WebSocketSession session) {
        ...
        sshConnectInfo.getRecorder().stop();
    }
    ...
    private void connectToSSH(SSHConnectInfo sshConnectInfo, WebSSHData webSSHData, WebSocketSession webSocketSession) throws JSchException, IOException {
        ...
        try {
            ...
            while ((i = inputStream.read(buffer)) != -1) {
                byte[] data = Arrays.copyOfRange(buffer, 0, i);
                sshConnectInfo.getRecorder().writeData(WebSSHRecordType.OUT, new String(data));
                sendMessage(webSocketSession, data);
            }
